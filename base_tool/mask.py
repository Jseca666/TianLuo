...
class MaskAdjuster(QtWidgets.QWidget):
    def __init__(self, adb_path=r"E:\leidian\LDPlayer9\adb.exe", device_address="127.0.0.1:5555",
                 config_path='../tool/masks.json'):
        super().__init__()
        self.adb_path = adb_path
        self.device_address = device_address
        self.config_path = config_path

        self.screenshot_dir = find_project_root() /'tool'/'screen_pic'/'tmp'
...
def main():
    ...
    window = MaskAdjuster(adb_path=adb_path, device_address=device_address, config_path='../tool/masks.json')
    window.show()
    sys.exit(app.exec_())
