...
class BackToMain:
    def __init__(self, device: AndroidDevice):
        ...
        self.info_file = self.project_root / 'tool' / 'location' / 'NIBO' / 'seven_info.json'
        self.img_file = self.project_root / 'tool'
    ...
    def back_to_main(self):
        ...
        # TUICHU_vivo_img = self.json_reader.img_path('退出主页提醒')
        quxiao_vivo_img = self.json_reader.img_path('vivo渠道服特别的退出标志')
        quxiao_mumu_img=self.json_reader.img_path('mumu模拟器版本特别的退出标志')
        # 持续按返回键，直到检测到图像
        while back:
            self.device.press_back_button()

            matches = self.device.find_image_on_screen(str(TUICHU_img))[0]
            vivo_mace=self.device.find_image_on_screen(str(quxiao_vivo_img))[0]
            mumu_mace = self.device.find_image_on_screen(str(quxiao_mumu_img))[0]
            if  matches != 0:
                ...
                print(f"找到区域 '退出主页提醒' 在位置: {vivo_mace}")
                time.sleep(2)
                quxiaoloc = self.device.wait_for_image(quxiao_vivo_img)[0]
                self.device.tap_on_screen(quxiaoloc)
                print('已回到主页')
                return True
            elif mumu_mace!=0:
                print(f"找到区域 '退出主页提醒' 在位置: {mumu_mace}")
                time.sleep(2)
                quxiaoloc = self.device.wait_for_image(quxiao_mumu_img)[0]
                self.device.tap_on_screen(quxiaoloc)
                print('已回到主页')
                return True
            else:
