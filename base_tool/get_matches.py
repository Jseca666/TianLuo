...
def get_matches(beijingjson,targetjson,device,beijing_name,target_name,mask_name=None,use_color=False,timeout=5,threshold=0.8):
    ...
    while time.time() - start_time < timeout:
        maches = device.find_image_in_screenshot(screenshot=beijinares,
                                                 ...
                                                 mask_name=mask_name, threshold=threshold, use_color=use_color)
        if maches != [0]:
            break
        else:
            time.sleep(interval)
    ...
    if maches!=[0]:
        ...
        rescult.append(0)
    return rescult
def get_wait_matches(targetjson,device,beijingjson,target_names,beijing_name=None,maskname=None,stop_on_first_match=False,thresholds=None,time_out=10,use_color=False):
    ...
    maches,parm_list = device.wait_for_images(beijinares=beijinares,
                                             ...
                                             mask_names=masknames, thresholds=thresholds,stop_on_first_match=stop_on_first_match,timeout=time_out,use_color=use_color)
    rescult = []
    # print('maches=',maches)
    # print(parm_list)
    if maches != [0]:
