...
class MainExportComparisonWorkflow:
    ...
    def compare(self, task_specs: Iterable[dict], output_root: Path) -> dict:
        ...
        quality = self.quality_facade.export(
            task_specs=task_specs,
            output_root=output_root / "quality",
            project_name="GeneratedAndroidProjectQuality",
            report_output_dir=output_root / "quality_reports",
        )
        ...
        summary = MainExportComparisonSummary(
            ...
            files={
                ...
                "quality_report": quality.get("quality_report_path", ""),
            },
            notes=[],
        )
        if summary.quality_todo_count < summary.baseline_todo_count:
            summary.notes.append("Quality export reduces TODO count")
        if summary.quality_unsupported_count < summary.baseline_unsupported_count:
            summary.notes.append("Quality export reduces unsupported count")
        if summary.quality_asset_file_count == summary.baseline_asset_file_count:
            summary.notes.append("Quality export keeps asset file count aligned with baseline")
