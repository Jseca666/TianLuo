...
class MainExportQualityRuntimeSemanticKotlinStepRenderer:
    def render(self, index: int, step: ExportedTaskStep) -> str:
        ...
        if action == "ocr_text":
            ...
            return f'''val locator{index} = context.locatorRepository.get("{locator}")
        val texts{index} = com.tianluo.runtime.template.runtime.ocr.readTextSemantic(
            context.ocrEngine,
            locator{index},
            {options},
        )'''
        ...
        if action == "ocr_int":
            ...
            options = self._ocr_options(params)
            return f'''val locator{index} = context.locatorRepository.get("{locator}")
        val number{index} = com.tianluo.runtime.template.runtime.ocr.readNumberSemantic(
            context.ocrEngine,
            locator{index},
            {options},
        )'''
        ...
        return f'// TODO main-export-quality-runtime-semantic: unsupported action: {action}'
